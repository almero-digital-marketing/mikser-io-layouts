        ...
        async ({ id, layout }) => {
            ...
            if (!layoutId) {
                ...
                if (consumers.length) {
                    ...
                    if (declared) {
                        ...
                        cmp = compare(entity, new Set([...req, ...opt]), opt)
                    } else {
                        cmp = compare(entity, new Set(read.keys), new Set())
                    }
                    ...
                    return ok({
                        ...
                        notes: [
                            ...
                            'missingFrom says where `missing` came from: "schema" is declared and authoritative; "consumers" means it is derived from what renders actually read off this entity, which only covers what someone happened to look at.',
                            'peerGaps compares this record with the others declaring the same type — keys most of them carry that this one lacks. For a data document this is often the only check with teeth, since everything else here depends on what a consumer happened to read. Weak evidence, never a failure.',
                ...
                return ok({
                    ...
                    notes: [
                        ...(orphanPeers?.gaps.length
                            ? ['peerGaps is the only check that could be made here: nothing reads this entity, so nothing defines what it needs, but the other records declaring the same type do. Weak evidence, never a failure — records differ for good reasons.']
                            : []),
            ...
            if (declaredSchema) {
                ...
                drift = {
                    ...
                    declaredButNotRead: [...req].filter(k => !isRead(k)).sort(),
                }
            }
            ...
            return ok({
                ...
                notes: [
                    'missing is the strong signal, but read missingFrom first. "schema" means a zod schema DECLARES the key required — authoritative, and it needed no template parsing. "inferred" means it was read out of the templates, which models one engine\'s semantics and has been wrong: a guard it cannot see makes a fine document look broken. Treat an inferred missing as strong evidence, not proof.',
                    'drift appears only with a schema, and is where the two sources disagree: readButNotDeclared is a key the layouts consume that nothing declares (a schema gap, or a typo in a template), declaredButNotRead is a declared key no layout reads (dead, or consumed over the API).',
                    ...
                    'peerGaps compares this record with the others declaring the same type: keys most of them carry that this one lacks, with the count as evidence. A layout contract cannot see this — a record missing a field its siblings all have renders perfectly and reads nothing that is absent. Weak evidence, and never a failure: records differ for good reasons.',
