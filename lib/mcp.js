    ...
    mcp.simpleTool(
        ...
        async ({ id, layout }) => {
            ...
            if (!layoutId) {
                ...
                const consumers = consumersOf(entity)
                if (consumers.length) {
                    return ok({
                        entity: id, kind: 'data',
                        ...
                        checked: false,
                        notes: [
                            `This entity never renders. ${consumers.length} render(s) pulled it in through a catalog query, so it is data, not a page — do not build expecting it to appear.`,
                            'No key-level contract yet: the engine records THAT a render queried this entity, not which of its fields that render then read. Until it does, the honest answer is the consumer list — open one and see what it reads.',
                            'A missing key here would surface as a hole in the pages listed above, not in this file.',
                        ],
                    })
                }
                return ok({
            ...
            const isOptional = (k) => optional.has(k)
                || [...optional].some(o => k.startsWith(`${o}.`) || k.startsWith(`${o}[`))
            ...
            const absent = [...consumed].filter(k => k && !isProvided(k))
