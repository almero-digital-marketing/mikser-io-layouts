// The tool surface for layouts. Registered against the ENGINE's tool
// registry, which exists whether or not the mcp plugin is installed — so
// there is no `if (!runtime.options.mcp) return` gate to write, and no rule
// about where mcp sits in the plugins array.
//
// The engine stores BARE names; the mcp plugin prefixes on the way out, so a
// session still sees `mikser_layouts_inspect` and the CLI sees
// `layouts_inspect`. One registration, both surfaces.
//
// Surfaces the inspect() primitive this package offers as the `layouts`
// service. The schema and author-facing description live here because
// they're MCP-flavor; the underlying data shape is owned by lib/inspect.js.

import { z } from 'zod'
import { registerTool, useService } from 'mikser-io'
...
export function registerMcpTools({ runtime, useLogger, findEntity, findEntities, useDatabase, collection }) {
    ...
    if (!layouts?.inspect) return   // our own factory has not run yet

    const logger = useLogger()
    registerTool('layouts_inspect',
        {
            description: 'Inspect a layout and everything it pulls in. Answers "what does this layout need from a document" '
        ...
        async ({ id, samples = 3 }) => {
            try {
                const result = await layouts.inspect(id, { samples })
                return {
                    content: [{
                        type: 'text',
                        text: JSON.stringify({
...
export function registerCheckTool({ runtime, findEntity, findEntities, useDatabase, collection, logger }) {
    // Resolved here rather than in the handlers: this runs from onLoaded, by
    // which point every factory — including our own, which provides it — has
    // run.
    const layouts = useService('layouts')
    ...
    registerTool('check_entity',
        {
            description: 'Check a document against the contract of the layout that renders it, BEFORE the mistake ships.\n\n'
        ...
        async ({ id, layout }) => {
            ...
            const inspected = await layouts.inspect(layoutId, {
                samples: 0,
            ...
            if (declaredSchema) {
                ...
                const wide = await layouts.inspect(layoutId, { samples: 0 })
