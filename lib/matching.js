...
import { reportWarning } from 'mikser-io'
...
export function createOnProcessed({
    ...
    function reportUnmatchedPatterns(logger) {
        ...
        const patterns = Object.keys(options.match || {})
        if (!patterns.length || !matchTally.evaluated) return
        matchTally.reported = true

        for (const pattern of patterns) {
            if (matchTally.hit.has(pattern)) continue
            ...
            reportWarning('layout-pattern-no-match', {
                pattern, layout: options.match[pattern], evaluated: matchTally.evaluated,
                matchedAgainst: onName ? 'name' : 'id',
            })
            logger.warn(
                'Layout pattern %j (→ %j) matched none of the %d entities evaluated. ' +
                'It is matched against entity.%s%s. On an incremental run unchanged ' +
                'entities are not re-evaluated; use --force to check the whole catalog.',
                pattern, options.match[pattern], matchTally.evaluated,
                onName ? 'name' : 'id',
        ...
        for await (let { entity, operation } of useJournal('Layouts processing', [OPERATION.CREATE, OPERATION.UPDATE, OPERATION.DELETE], signal)) {
            ...
            switch (operation) {
                ...
                case OPERATION.UPDATE:
                    ...
                    //
                    // entity.destination used to exist ONLY as a field the
                    // render-task builder set per task, and was never written
                    // back — so the catalog had none, and runtime.href()
                    // (which needs the TARGET's destination to make a
                    // relative URL) fell through and returned the whole
                    // entity instead of { url }. The catalog is only written
                    // through the journal, and this hook is where that
                    // happens, so it has to be derived here.
                    //
