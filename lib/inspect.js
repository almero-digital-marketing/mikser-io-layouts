...
async function buildContract({ layout, templateSource, runtime, findEntities, collection, only }) {
    ...
    async function walk(name, template, source, bindings, depth) {
        ...
        for (const a of parsed.assigns ?? []) {
            // A DERIVED value is not an alias. `{% assign rows = c.specs |
            // split: '|' %}` turns a string into a list, so a read of `rows[0]`
            // says nothing about `specs` having members — binding it would
            // report `specs[]` as a required key of a document whose `specs` is
            // the string being split, and no document can satisfy that.
            //
            // The source path is still recorded by the parser as consumed,
            // which is the true dependency.
            if (a?.key && a.from && !a.derived) subs[a.key] = resolve(a.from, subs)
        }
